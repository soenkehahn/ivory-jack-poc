...
main :: IO ()
main = C.runCompiler [package "foo" fooModule] [] C.initialOpts{
  C.outDir = Just "."
}

fooModule :: ModuleM ()
fooModule = runWithUniqueNames $ do
...
mkProcessor :: (IvoryInit a, IvoryZeroVal a, Num a) =>
     String
  -> (Signal a -> Body ())
  -> M (Signal a)
...
constant :: (IvoryInit a, Num a, IvoryStore a, IvoryZeroVal a) =>
  a -> M (Signal a)
constant c = mkProcessor "constant" $ \ (Signal out) -> body $ do
  store out c

mult :: (IvoryInit a, Num a, IvoryStore a, IvoryZeroVal a) =>
